...
void WindowsPacker::pack(const QString &path)
{
    if (mThreadPacking)
    {
...
void WindowsPacker::threadPack(const QString &path)
{
    ...
    fp = _popen(QString("windeployqt %1").arg(fileInfo.absoluteFilePath()).toStdString().data(), "r");
    if(fp)
    {
    ...
    for (int64_t i = 0; i < size; i++)
    {
        ...
        if (QFile::exists(QString("%1/%2").arg(fileInfo.absolutePath(), info.fileName()))) continue;
